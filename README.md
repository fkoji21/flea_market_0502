...
Docker による環境構築や、メール証言付きログインも実装済みです。
...
- Fortify を用いた会員登録・ログイン・ログアウト機能 (メール証言付き)
...
- 「カード支払い」「コンビニ支払い」の選択後、**Stripe の checkout 経由で決済画面へ接続**される動線を実装しています
- 本アプリは決済画面までの動線を保証しており、**実際の購買、貯金は行われません** (デモ用)
